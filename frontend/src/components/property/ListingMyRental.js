...
const ListingItem = (props) => {
    ...
    const makePayment = async () => {
        ...
        const payment = parseFloat(price) + parseFloat(downPaymentPrice);
        const tokenIdOfThisProperty = tokenID;
        await arcanaProvider.connect();
        if (arcanaProvider.provider.connected) {
            const amountInWei = utils.parseUnits(payment.toString(), 18);

            // call the pay to mint method on the smart contract
            const result = await contract.buyerPayRentAndDeposit(
